import os, random, h5py, time, math, cmath, copy, importlib, warnings
...
def get_behavior(parameter):
  
  for file in os.listdir(parameter['pathSession']):
    if file.endswith("aligned.mat"):
        pathBH = os.path.join(parameter['pathSession'], file)
  print('first, check alignment again!! (make sure, there is no misalignment, adjust bin numbers to 100)')
  ...
  data_behavior = {}
  data_behavior['longrunperiod'] = load_behavior['longrunperiod'].astype(bool)
  data_behavior['binpos'] = load_behavior['binpos'][data_behavior['longrunperiod']].astype('int') - 1 ## correct for different indexing
  data_behavior['binpos_raw'] = load_behavior['binpos'].astype('int') - 1 ## correct for different indexing
  data_behavior['binpos_coarse'] = (data_behavior['binpos']/parameter['coarse_factor']).astype('int')
  data_behavior['time'] = load_behavior['time'][data_behavior['longrunperiod']]
  data_behavior['time_raw'] = load_behavior['time']
  data_behavior['T'] = np.count_nonzero(data_behavior['longrunperiod'])
  
  #plt.figure()
  #plt.plot(data_behavior['time'],data_behavior['binpos_coarse'],'r')
  #plt.plot(data_behavior['time'],data_behavior['binpos'],'b')
  #plt.show()
  
  ###### define trials
  data_behavior['trials'] = {}
  data_behavior['trials']['frame'] = np.hstack([0, np.where(np.diff(data_behavior['binpos'])<-10)[0]+1,len(data_behavior['time'])-1])
  data_behavior['trials']['t'] = np.hstack([data_behavior['time'][data_behavior['trials']['frame'][:-1]],data_behavior['time'][-1]])
  dt = np.diff(data_behavior['trials']['t'])
  ct = len(data_behavior['trials']['frame'])-1
  if dt[0] < 2:
    data_behavior['trials']['t'] = np.delete(data_behavior['trials']['t'],0)
    data_behavior['trials']['frame'] = np.delete(data_behavior['trials']['frame'],0)
    ct -=1
  if dt[-1] < 2:
    data_behavior['trials']['t'] = np.delete(data_behavior['trials']['t'],ct)
    data_behavior['trials']['frame'] = np.delete(data_behavior['trials']['frame'],ct)
    ct -=1
  
  data_behavior['trials']['ct'] = ct
  data_behavior['trials']['dwelltime'] = np.zeros((data_behavior['trials']['ct'],parameter['nbin']))
  data_behavior['trials']['T'] = np.zeros(data_behavior['trials']['ct']).astype('int')
  
  t_offset = 0
  data_behavior['trials']['trial'] = {}
  for t in range(data_behavior['trials']['ct']):
    data_behavior['trials']['trial'][t] = {}
    data_behavior['trials']['trial'][t]['binpos'] = data_behavior['binpos'][data_behavior['trials']['frame'][t]:data_behavior['trials']['frame'][t+1]]
    data_behavior['trials']['dwelltime'][t,:] = np.histogram(data_behavior['trials']['trial'][t]['binpos'],parameter['bin_array_centers'])[0]/parameter['f']
    data_behavior['trials']['T'][t] = len(data_behavior['trials']['trial'][t]['binpos'])
...
#def PC_detect(varin):
  ...
  result = build_PC_fields()
  S[S<0] = 0
  
  #try:
  active = {}
  active['S'] = S[data_behavior['longrunperiod']]    ### only consider activity during continuous runs
  ...
  #plt.scatter(data_behavior['time'],data_behavior['binpos'],marker='.',s=3)
  #S_plot = S/(md+2*sd_r);
  #plt.scatter(data_behavior['time'][active['S']>1],data_behavior['binpos'][active['S']>1],color='r',s=10+S_plot[data_behavior['longrunperiod']&(S>(md+2*sd_r))]*2)
  #plt.subplot(211)
  ...
  result['firingstats']['rate'] = spikeNr / (data_behavior['T']/parameter['f'])
  
  if parameter['modes']['info'] == 'MI':
    ## obtain quantized firing rate for MI calculation
    active['qtl'] = sp.ndimage.gaussian_filter(S,parameter['sigma'])
    active['qtl'] = active['qtl'][data_behavior['longrunperiod']]
    qtls = np.quantile(active['qtl'][active['qtl']>0],np.linspace(0,1,parameter['qtl_steps']+1))
    active['qtl'] = (active['qtl'][:,np.newaxis]>=qtls[np.newaxis,1:-1]).sum(1)
  
  ### get trial-specific activity
  trials_S, trials_firingmap = get_trials_activity(active)
  
  ## obtain firingmap
  firingstats_tmp = get_firingstats_from_trials(trials_firingmap)
  ...
  MI_tmp = test_MI(active,trials_S)
  for key in MI_tmp.keys():
    result['status'][key] = MI_tmp[key]
  
  ### do further tests only if there is "significant" mutual information
  if result['status']['MI_p_value']<0.2:
    
    #### -------------------------- tuning curve model -------------------------------------
    #PC_fields['firingmap'] = savgol_filter(PC_fields['firingmap'], 5, 3)
    #parameter['bin_array'] = np.linspace(2*math.pi/parameter['nbin'],2*math.pi,parameter['nbin']);
    
    hbm = HierarchicalBayesModel(result['firingstats']['map'],parameter['bin_array'],result['firingstats']['parNoise'],0)
    ...
    for f in range(2):
      ...
      sampler = ultranest.ReactiveNestedSampler(parameter['names'][:hbm.nPars], my_likelihood, my_prior_transform,wrapped_params=hbm.pTC['wrap'],vectorized=True,num_bootstraps=30)#,log_dir='/home/wollex/Data/Documents/Uni/2016-XXXX_PhD/Japan/Work/Programs/PC_analysis/test_ultra')   ## set up sampler...
      ...
      if f > 0:
        ...
        #if bayes_thr > 1:
        fields_tmp = detect_modes_from_posterior(sampler)
        for key in fields_tmp.keys():
          result['fields'][key] = fields_tmp[key]
          
        #if parameter['plt_bool']:
    ...
    if parameter['plt_bool']:
      ...
      ax.bar(parameter['bin_array'],result['firingstats']['map'],facecolor='b',width=100/parameter['nbin'],alpha=0.2)
      ax.errorbar(parameter['bin_array'],result['firingstats']['map'],result['firingstats']['CI'],ecolor='r',linestyle='',fmt='',elinewidth=0.3)#,label='$95\\%$ confidence')
      
      ax.plot(parameter['bin_array'],hbm.TC(np.array([A_0])),'k',linestyle='--',linewidth=1,label='(constant)\t $log(Z)=%5.2g\\pm%5.2g$'%(result['status']['Z'][0,0],result['status']['Z'][0,1]))
      ...
      for c in range(min(2,result['fields']['nModes'])):
        ...
        ax.plot(parameter['bin_array'],hbm.TC(result['fields']['parameter'][c,:,0]),'r',linestyle='-',linewidth=0.5+result['fields']['posterior_mass'][c]*2,label=label_str)
        #except:
          #1
      #ax.plot(parameter['bin_array'],hbm.TC(par_results[1]['mean']),'r',label='$log(Z)=%5.3g\\pm%5.3g$'%(par_results[1]['Z'][0],par_results[1]['Z'][1]))
      ax.legend(title='evidence from nested sampling')
      ax.set_xlabel('Position on track')
      ax.set_ylabel('Ca$^{2+}$ event rate')
      plt.tight_layout()
      if parameter['plt_sv']:
        pathSv = pathcat([parameter['pathFigs'],'PC_analysis_fit_results.png'])
        plt.savefig(pathSv)
        print('Figure saved @ %s'%pathSv)
      plt.show(block=True)
  
  print('p-value (MI): %5.3g, z-score (MI): %5.3g, \t bayes factor: %7.5g+/-%7.5g \t time passed: %7.2fs'%(result['status']['MI_p_value'],result['status']['MI_z_score'],result['status']['Bayes_factor'][0],result['status']['Bayes_factor'][1],time.time()-t_start))
  #global n_ct
  #print('tried')
  #n_ct.increment()
  ...
  #except (KeyboardInterrupt, SystemExit):
    #raise
  #except:
    #print('analysis failed: (-) p-value (MI): %5.3g, \t bayes factor: %7.5g+/-%7.5g \t time passed: %7.2fs'%(result['status']['MI_p_value'],result['status']['Bayes_factor'][0],result['status']['Bayes_factor'][1],time.time()-t_start))
    #result['fields']['nModes'] = -1
    
  return result


def get_fields(basePath,mouse,s,nP,plt_bool=False,sv_bool=False,specific_n=False):
  
  print('stopped at m884, s16')
  print('proper # of quantiles?')
  ...
  global parameter
  global data_behavior
  parameter, data_behavior = prepare_PC_detect(pathSession,nP,plt_bool,sv_bool)
  ...
  pathAct = pathcat([parameter['pathSession'],'results_OnACID.mat'])
  S = sio.loadmat(pathAct)
  S = S['S']
  nCells = S.shape[0]
  if nCells > 8000:
    S = S.transpose()
  ...
  if nP > 0:
    ...
    pool = mp.Pool(nP)
    batchSz = 100
    nBatch = nCells//batchSz
    for i in range(nBatch+1):
      result.extend(pool.map(PC_detect,S[i*batchSz:min(nCells,(i+1)*batchSz),:]))
      #result.extend(pool.map(PC_detect,pool_dat[i*batchSz:min(nCells,(i+1)*batchSz)]))
      print('\t\t\t ------ mouse %s --- session %d ------ %d neurons processed\t ------ \t time passed: %7.2fs'%(mouse,s,min(nCells,(i+1)*batchSz),time.time()-t_start))
  else:
    #global parameter
    #global data_behavior
    for n in range(nCells):
      result.append(PC_detect(S[n,:]))
      #result.append(PC_detect({'S':S[n,:],'data_bh':data_behavior,'parameter':parameter}))
      print('\t\t\t ------ mouse %s --- session %d ------ %d neurons processed\t -----\t time passed: %7.2fs'%(mouse,s,n+1,time.time()-t_start))
  
  results = build_PC_results(nCells)   ## pre-allocate array
  for (r,n) in zip(result,range(nCells)):
  ...
  savemat(parameter['svname_status'],results['status'])
  savemat(parameter['svname_fields'],results['fields'])
  savemat(parameter['svname_firingstats'],results['firingstats'])
...
def detect_modes_from_posterior(sampler):
  ### handover of sampled points
  data_tmp = ultranest.netiter.logz_sequence(sampler.root,sampler.pointpile)
  ...
  ### for each cluster, obtain parameter values and confidence intervals
  fields = {}
  fields['nModes'] = min(3,c_ct)
  fields['posterior_mass'] = np.zeros(3)*np.NaN
  fields['parameter'] = np.zeros((3,4,1+len(parameter['CI_arr'])))*np.NaN
  fields['p_x'] = np.zeros((3,2,parameter['nbin']))*np.NaN
  ...
  for c in clusters.keys():
    if clusters[c]['active']:
      ...
      for i in range(4):
        ...
        for j in range(len(parameter['CI_arr'])):
          fields['parameter'][c_ct,i,1+j] = x_cdf_posterior[np.where(y_cdf_posterior>=parameter['CI_arr'][j])[0][0]]
  ...
  if parameter['plt_bool']:
    ...
    if parameter['plt_sv']:
        pathSv = pathcat([parameter['pathFigs'],'PC_analysis_NS_results.png'])
        plt.savefig(pathSv)
        print('Figure saved @ %s'%pathSv)
    plt.show(block=False)
...
def get_trials_activity(active):
  
  ## preallocate
  trials_map = np.zeros((data_behavior['trials']['ct'],parameter['nbin']))
  #trials_rate = np.zeros(data_behavior['trials']['ct'])
  
  trials_S = {}
  for t in range(data_behavior['trials']['ct']):
    trials_S[t] = {}
    trials_S[t]['S'] = active['S'][data_behavior['trials']['frame'][t]:data_behavior['trials']['frame'][t+1]];    ## should be quartiles?!
    if parameter['modes']['info'] == 'MI':
      trials_S[t]['qtl'] = active['qtl'][data_behavior['trials']['frame'][t]:data_behavior['trials']['frame'][t+1]];    ## should be quartiles?!
    
    if parameter['modes']['activity'] == 'spikes':
    ...
    trials_S[t]['rate'] = trials_S[t]['S'].sum()/(data_behavior['trials']['T'][t]/parameter['f']);
    
    if trials_S[t]['rate'] > 0:
      trials_map[t,:] = get_firingmap(trials_S[t]['S'],data_behavior['trials']['trial'][t]['binpos'],data_behavior['trials']['dwelltime'][t,:],False)/trials_S[t]['rate']
    
    
    #[spikeNr,md,sd_r] = get_spikeNr(trials_S[t]['S'][trials_S[t]['S']>0]);
    #trials_rate[t] = spikeNr/(data_behavior['trials']['T'][t]/parameter['f']);
    
  return trials_S, trials_map#, trials_rate


def get_firingstats_from_trials(trials_firingmap):
  
  firingstats = {}
  ### construct firing rate map from bootstrapping over trials (is that valid? what does it imply?)
  firingmap_bs = np.zeros((parameter['nbin'],parameter['N_bs']))
  for L in range(parameter['N_bs']):
    base_sample = np.random.randint(0,data_behavior['trials']['ct'],(data_behavior['trials']['ct'],))
    dwelltime = data_behavior['trials']['dwelltime'][base_sample,:].sum(0)
  ...
  firingstats['map'][firingstats['map']<=0] = 1/data_behavior['T']     ## set 0 firing rates to lowest possible (0 leads to problems in model, as 0 noise, thus likelihood = 0)
  ...
  if parameter['plt_theory_bool']:
    plt_model_selection(firingmap_bs,firingstats['parNoise'],trials_firingmap)
  
  return firingstats


def get_info_value(activity, dwelltime):
  
  if parameter['modes']['info'] == 'MI':
    
    t_joint = time.time()
    p_joint = get_p_joint(activity)   ## need activity trace
    t_MI = time.time()
    MI = get_MI(p_joint,dwelltime,parameter['qtl_weight'])
    firingmap = []
    
  #elif parameter['modes']['info'] == 'Isec':
    
    #firingmap = imgaussfilt(get_firingmap(activity,binpos,dwelltime,parameter,True),parameter['sigma'],
                            #'Padding','circular','FilterDomain','spatial')
    #MI = get_I_sec(firingmap,dwelltime,parameter)
...
def get_p_joint(activity):
  ...
  ### - behavior trace
  p_joint = np.zeros((parameter['nbin_coarse'],parameter['qtl_steps']))
  for f in range(parameter['qtl_steps']):
    for (x,ct) in Counter(data_behavior['binpos_coarse'][activity==f]).items():
      p_joint[x,f] = ct;
...
def test_MI(active,trials_S):
  ...
  rand_distr = np.zeros(parameter['repnum'])*np.NaN
  
  ### first, get actual MI value
  norm_dwelltime = data_behavior['trials']['dwelltime'].sum(0)*parameter['f']/data_behavior['T']
  norm_dwelltime_coarse = np.zeros(parameter['nbin_coarse'])
  for i in range(parameter['nbin_coarse']):
    norm_dwelltime_coarse[i] = norm_dwelltime[i*parameter['coarse_factor']:(i+1)*parameter['coarse_factor']].sum()
  MI['MI_value'] = get_info_value(active[S_key],norm_dwelltime_coarse);    
  
  ### shuffle according to specified mode
  #t_start_shuffle = time.time()
  
  for L in range(parameter['repnum']):
    
    ## shift single trials to destroy characteristic timescale
    if parameter['modes']['shuffle'] == 'shuffle_trials':
      
      ## trial shuffling
      shuffled_activity = np.zeros(data_behavior['T'])
      idx = 0
      for t in np.random.permutation(data_behavior['trials']['ct']):
        if np.nonzero(trials_S[t][S_key]):
          shuffled_activity[idx:idx+data_behavior['trials']['T'][t]] = shuffling('shift',shuffle_peaks,spike_train=trials_S[t][S_key])
        else:
          shuffled_activity[idx:idx+data_behavior['trials']['T'][t]] = trials_S[t][S_key]
        
        idx = idx+data_behavior['trials']['T'][t]
      ...
      if parameter['modes']['activity'] == 'spikes':
        ...
        shuffled_activity = shuffling('dithershift',shuffle_peaks,spike_times=spike_times_L,spikes=spikes_L,T=data_behavior['T'],ISI=ISI_L,w=2*parameter['f'])
      else:
        shuffled_activity = shuffling('shift',shuffle_peaks,spike_train=shuffled_activity)
      
    elif parameter['modes']['shuffle'] == 'shuffle_global':
      if parameter['modes']['activity'] == 'spikes':
        shuffled_activity = shuffling('dithershift',shuffle_peaks,spike_times=spike_times,spikes=spikes,T=data_behavior['T'],ISI=ISI,w=2*parameter['f'])
      else:
        shuffled_activity = shuffling('shift',shuffle_peaks,spike_train=active[S_key])
      
    elif parameter['modes']['shuffle'] == 'randomize':
      shuffled_activity = active[S_key][np.random.permutation(len(active[S_key]))];
    #
    #t_start_info = time.time()
    #print(shuffled_activity)
    rand_distr[L] = get_info_value(shuffled_activity,norm_dwelltime_coarse)
  ...
  if MI['MI_value'] > rand_distr.max():
    MI['MI_p_value'] = 1/parameter['repnum']
  else:
    x,y = ecdf(rand_distr)
...
def build_PC_fields():
  ...
  result['fields'] = {'parameter':np.zeros((3,4,1+len(parameter['CI_arr'])))*np.NaN,          ### (field1,field2) x (A0,A,std,theta) x (mean,CI_low,CI_top)
                      'posterior_mass':np.zeros(3)*np.NaN,
                      'p_x':np.zeros((3,2,parameter['nbin']))*np.NaN,
                      'nModes':0}
  
  result['firingstats'] = {'rate':np.NaN,
                          'map':np.zeros(parameter['nbin'])*np.NaN,
                          'std':np.zeros(parameter['nbin'])*np.NaN,
                          'CI':np.zeros((2,parameter['nbin']))*np.NaN,
                          'parNoise':np.NaN}
  return result
  
  
def build_PC_results(nCells):
  results = {}
  ...
  results['fields'] = {'parameter':np.zeros((nCells,3,4,1+len(parameter['CI_arr'])))*np.NaN,          ### (mean,std,CI_low,CI_top)
                       'p_x':np.zeros((nCells,3,2,parameter['nbin'])),
                       'posterior_mass':np.zeros((nCells,3))*np.NaN,
                       'nModes':np.zeros(nCells).astype('int')}
  
  results['firingstats'] = {'rate':np.zeros(nCells)*np.NaN,
                            'map':np.zeros((nCells,parameter['nbin']))*np.NaN,
                            'std':np.zeros((nCells,parameter['nbin']))*np.NaN,
                            'CI':np.zeros((nCells,2,parameter['nbin']))*np.NaN,
                            'parNoise':np.zeros(nCells)*np.NaN}
  
  return results


def prepare_PC_detect(pathSession,nP,plt_bool,sv_bool):
  
  ## set paths:
  nbin = 100
  qtl_steps = 5
  coarse_factor = 4
  parameter = {'nbin':nbin,'f':15,
  ...
  data_behavior = get_behavior(parameter)
  
  return parameter, data_behavior


def plt_model_selection(fmap_bs,parsNoise,trials_fmap):
  ...
  ax.bar(parameter['bin_array'],fr_mu,color='b',alpha=0.2,width=1,label='$\\bar{\\nu}$')
  ax.errorbar(parameter['bin_array'],fr_mu,fr_CI,ecolor='r',elinewidth=0.5,linestyle='',fmt='',label='$95\\%$ confidence')
  ...
  #ax1 = plt.subplot(211)
  ax1.bar(parameter['bin_array'],fr_mu,color='b',alpha=0.2,width=1,label='$\\bar{\\nu}$')
  ax1.errorbar(parameter['bin_array'],fr_mu,fr_CI,ecolor='r',linestyle='',fmt='',label='$95\\%$ confidence')
  Y = trials_fmap/data_behavior['trials']['dwelltime']
  mask = ~np.isnan(Y)
  Y = [y[m] for y, m in zip(Y.T, mask.T)]
  
  #flierprops = dict(marker='.', markerfacecolor='k', markersize=0.5)
  #h_bp = ax1.boxplot(Y,flierprops=flierprops)#,positions=parameter['bin_array'])
  ...
  ax1.legend(title='# trials = %d'%data_behavior['trials']['ct'],fontsize=12)#[h_bp['boxes'][0]],['trial data'],
  ...
  i = random.randint(0,parameter['nbin']-1)
  ax1.bar(parameter['bin_array'][i],fr_mu[i],color='b')
  ...
  D_KL_gamma = np.zeros(parameter['nbin'])
  D_KL_gauss = np.zeros(parameter['nbin'])
  D_KL_lognorm = np.zeros(parameter['nbin'])
  
  for i in range(parameter['nbin']):
    x_arr = np.linspace(0,fmap_bs[i,:].max()*1.2,101)
    act_hist = np.histogram(fmap_bs[i,:],x_arr,normed=True)
    alpha, beta = gamma_paras(fr_mu[i],fr_std[i])
    mu, shape = lognorm_paras(fr_mu[i],fr_std[i])
  ...
  plt.tight_layout()
  if parameter['plt_sv']:
    pathSv = pathcat([parameter['pathFigs'],'PC_analysis_get_noise.png'])
    plt.savefig(pathSv)
    print('Figure saved @ %s'%pathSv)
  plt.show(block=False)
...
def get_firingmap(S,binpos,dwelltime,bool_norm):
  ...
  #if not bool_norm:
    #dwelltime = np.histogram(binpos,parameter['bin_array_centers'])[0]/parameter['f']
  #else:
