...
class cluster_parameters:
    ...
    def set_paths(self, paths_session, paths_results, pathMouse, suffix=""):
        ...
        # pathAssignments = os.path.join(
        #     pathMouse,
        #     f'matching/neuron_registration{suffix}.{"mat" if self.matlab else "pkl"}',
        # )
        ...
        self.paths = {
            ...
            # "neuron_detection": paths_results,
            # "sessions": paths_session,
            "suffix": suffix,
            # 'data':                   pathData,
        ...
        # try:
        #     while True:
        #         pathPCFields = os.path.join(
        #             paths_session[s], self.paths["fileNamePCFields"]
        #         )
...
def replace_relative_path(paths, newPath):
    ...
    return [
        os.path.join(newPath, os.path.relpath(path, prepath)) if path else ""
        for path in paths
    ]
