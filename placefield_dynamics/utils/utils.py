...
def periodic_distr_distance(
    p1, p2, nbin, mu1=None, mu2=None, N_bs=1000, mode="wasserstein"
):

    steps = p1.shape[-1]
    bin_to_step = steps / nbin

    x = np.linspace(0, nbin, steps)
    ...
    if mode == "wasserstein":
        ...
        if abs(d_raw) > nbin / 2:
            shift = int((mu1 + mu2) / 2)
            d_out = sstats.wasserstein_distance(
                x,
                x,
                np.roll(p1, shift * bin_to_step),
                np.roll(p2, shift * bin_to_step),
                # (mu1 + shift) % nbin,
                # (mu2 + shift) % nbin,
            )
        else:
            ...
            if (
                ...
                for shift in range(steps):
                    ...
                    d[shift] = sstats.wasserstein_distance(
                        x, x, np.roll(p1, shift), np.roll(p2, shift)
                    )
                d_out = d.min()  # / bin_to_step
        ...
        x1 = np.argmin(abs(rnd1 - cdf1[np.newaxis, :]), 1) / bin_to_step
        x2 = np.argmin(abs(rnd2 - cdf2[np.newaxis, :]), 1) / bin_to_step
        ...
        for i in np.where(x1 == 0)[0]:
            tmp = abs(rnd1[i] - cdf1)
            x1[i] = np.where(tmp == tmp.min())[0][-1] / bin_to_step
        for i in np.where(x2 == 0)[0]:
            tmp = abs(rnd2[i] - cdf2)
            x2[i] = np.where(tmp == tmp.min())[0][-1] / bin_to_step
        shift_distr = np.zeros(N_bs)

        ## generate two samples from distribution from cdfs
        shift_distr = (x2 - x1 + nbin / 2) % nbin - nbin / 2
        d_out = get_average(shift_distr, 1, periodic=True, bounds=[-nbin / 2, nbin / 2])
        p_out = np.histogram(
            shift_distr, np.linspace(-nbin / 2, nbin / 2, nbin + 1), density=True
        )[0]
        p_out[p_out < 10 ** (-10)] = 0
...
def fit_plane(data, anchor=None):
    ...
    assert (
        data.shape[-1] == 3
    ), "Please provide the values of x,y and z-direction for each data point"
...
def com(A, d1, d2, d3=None):
    ...
    Anorm = sp.sparse.vstack(
        [
            (
                a.multiply(a > 0.001 * a.max()) / a[a > 0.001 * a.max()].sum()
                if (a > 0).sum() > 0
                else sp.sparse.csc_matrix(a.shape)
            )
            for a in A.T
        ]
    ).T
...
def calculate_img_correlation(
    ...
    if shift:
        ...
        if crop:
            ...
            if padding[0] > 0:
                A1 = np.pad(
                    A1, [[padding[0], 0], [0, 0]], mode="constant", constant_values=0
                )
            else:
                A2 = np.pad(
                    A2, [[-padding[0], 0], [0, 0]], mode="constant", constant_values=0
                )

            if padding[1] > 0:
                A1 = np.pad(
                    A1, [[0, 0], [padding[1], 0]], mode="constant", constant_values=0
                )
            else:
                A2 = np.pad(
                    A2, [[0, 0], [-padding[1], 0]], mode="constant", constant_values=0
                )
            # t_end = time.time()
        ...
        # if not crop:
        crop_half = (
            (dims[0] - np.mod(dims[0], 2)) / 2,
            (dims[1] - np.mod(dims[1], 2)) / 2,
        )  # tuple(int(d/2-1) for d in dims)
        ...
        if plot_bool:
            ...
            plt.subplot(221)
            plt.imshow(
                A1.reshape(extent[1, 0] - extent[0, 0], extent[1, 1] - extent[0, 1]),
                origin="lower",
            )
            plt.colorbar()
            plt.subplot(222)
            plt.imshow(
                A2.reshape(extent[1, 0] - extent[0, 0], extent[1, 1] - extent[0, 1]),
                origin="lower",
            )
            plt.colorbar()
            # plt.subplot(223)
            ...
            plt.show(block=False)
        return (
            A1.multiply(A2).sum() / np.sqrt(A1.power(2).sum() * A2.power(2).sum()),
            None,
        )
...
def get_shift_and_flow(
    A1, A2, dims=(512, 512), projection=-1, transpose_it=False, plot_bool=False
):
...
def get_status_arr(cluster, SD=1):
    ...
    status["act"] = np.pad(
        cluster.status["activity"][..., 1][..., np.newaxis],
        ((0, 0), (0, 0), (0, nSes - 1)),
        mode="edge",
    )
    status["code"] = np.pad(
        cluster.status["activity"][..., 2][..., np.newaxis],
        ((0, 0), (0, 0), (0, nSes - 1)),
        mode="edge",
    )
