...
class cluster_analysis:
	...
	def run_complete(self,sessions=None,n_processes=0,reprocess=False):
		...
		self.stability = self.calculate_placefield_stability(dsMax=self.data['nSes'],n_processes=n_processes,N_bs=100)
	...
	def prepare_dicts(self,which=None,overwrite=False):
		...
		if 'fields' in which and (not hasattr(self,'fields') or overwrite):
			self.fields = {
				...
				'Bayes_factor':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max']),np.NaN),
				'reliability':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max']),np.NaN),
				'max_rate':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max']),np.NaN),
				'posterior_mass':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max']),np.NaN),
				'baseline':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max'],3),np.NaN),
				'amplitude':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max'],3),np.NaN),
				'width':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max'],3),np.NaN),
				'location':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max'],3),np.NaN),
				'p_x':np.full((self.data['nC'],self.data['nSes'],self.params['field_count_max'],self.data['nbin']),np.NaN)
			}
	...
	def update_status(self,complete=True,
		...
		self.thr = {
			...
			'r_values':     matching_params['rval_lowest'] if rval_thr is None else rval_thr,
					...
					for s in range(self.data['nSes']):
						...
						if self.status['activity'][c,s,2]:
							...
							for f in np.where(self.status_fields[c,s,:])[0]:#range(self.fields['nModes'][c,s]):
								...
								self.status['activity'][c,s,self.fields['status'][c,s,f]] = True
	...
	@timing
	def calculate_placefield_stability(self,dsMax,celltype='all',n_processes=8,N_bs=100,p_keys=['all','cont','mix','discont','silent_mix','silent']):
		...
		s1_shifts,s2_shifts,_,_ = np.unravel_index(self.compare['pointer'].col,(nSes,nSes,self.params['field_count_max'],self.params['field_count_max']))
		...
		if n_processes>1:
			...
			res = pool.starmap(get_shift_distribution,zip(range(1,dsMax),itertools.repeat(self.compare),itertools.repeat((nSes,nbin,N_bs,idx_celltype,s1_shifts,s2_shifts,p_keys))))
			pool.close()
		else:
			res = []
			for ds in range(1,dsMax):
				res.append(get_shift_distribution(ds,self.compare,(nSes,nbin,N_bs,idx_celltype,s1_shifts,s2_shifts,p_keys)))
		...
		if 'p_post_s' in which:
			...
			for status_key in status_arr:
				...
				for status2_key in status_arr:
					...
					for p_post in p_posts:
						self.stats[f'p_post{p_post}_s'][status_key][status2_key] = np.full((nSes,ds_max+1,2),np.NaN)
					self.stats['p_pre_s'][status_key][status2_key] = np.full((nSes,ds_max+1,2),np.NaN)
						...
						for key in status_arr:
							for key2 in status_arr:
								...
								if s-ds>=0 and self.status['sessions'][s-ds]:
									...
									self.stats['p_pre_s'][key][key2][s,ds,0] = np.nanmean(status_pos)
									status_neg = status[key2][(~status[key][:,s,1]) & status_dep[key][:,s] & status_dep[key2][:,s-ds],s-ds,ds]
									self.stats['p_pre_s'][key][key2][s,ds,1] = np.nanmean(status_neg)
...
def bootstrap_shifts(fun,shifts,N_bs,nbin):
    ...
    N_data = len(shifts)
    if N_data == 0:
        return np.full(4,np.NaN),np.full((2,4),np.NaN),np.full(4,np.NaN),np.full((2,nbin),np.NaN)
