    ...
    if plot_fig[27]:
        ...
        for j,s in enumerate(s_arr):
            if s < nSes:
                ...
                idxes_tmp = np.where(cluster.status_fields[:,s,:] & (cluster.stats['SNR_comp'][:,s]>2)[...,np.newaxis] & (cluster.stats['r_values'][:,s]>0)[...,np.newaxis] & (cluster.matching['score'][:,s,0]>0.5)[...,np.newaxis])
                idxes = idxes_tmp[0]
