...
// 
// History:   Random.c  17 Nov 1994 - 8 Jul 2004
//
// Acknowledgments:
...
// Purpose:   Draw sample from 
//
//                prob(x) = exp( - c * cosh(x) ) / Z    for c > 0
// 
// History:   JS         8 Jul 2004
//-----------------------------------------------------------------------------
// 
double  Ranexpcosh(    //   O  Value
Rand_t   Rand,         // I O  Random generator state
double   c)            // I    Coefficient
{
    double x, m, h;
    double big = log(DBL_MAX);

// Bound  exp(-c * cosh(t))  by  exp(h - m|t|)  with  h and m
// set so that bounding area is within about 7 percent of minimal.
    x = (c < 0.4) ? log(2.0 / (c * log(2.0 / c))) : 1.0 / sqrt(c);
    m = c * sinh(x);
    h = m * x - c * cosh(x);
// Sample positive t from bounding exponential
    do  x = -log(Randouble(Rand)) / m;
    while( x > big || log(Randouble(Rand)) > m * x - h - c * cosh(x) );
// Random sign
    return (Randouble(Rand) < 0.5) ? -x : x;
}
