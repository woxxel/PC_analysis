...
do
  ...
  do
    ...
    if test -d $session_path/images; then
        ...
        echo "$datapath_out/$dataset/$mouse/$session_name/log_silence_redetection.out"
...
python3 ./silence_redetection_hpc_wrapper.py $datapath_in $datapath_out $dataset $mouse $s $session_name $cpus
EOF
