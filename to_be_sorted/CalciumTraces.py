...
class CalciumTraces:
    ...
    def load_data(self, C=None, S=None, SNR=None, r_values=None):
        ...
        if C is None:
            ...
            ld = loadmat(
                pathResults,
                variable_names=["C", "S", "A", "b", "f", "SNR", "r_values", "CNN"],
                squeeze_me=True,
            )
            for key in ld.keys():
    ...
    def plot_stats(self, sig=2, SNR_thr=2, r_thr=0):
        ...
        self.N_spikes_C = np.floor(
            self.data["C"]
            / (self.baseline_C[:, np.newaxis] + sig * self.noise_C[:, np.newaxis])
        ).sum(1)

        self.N_spikes_S = np.floor(
            self.data["S"]
            / (self.baseline_S[:, np.newaxis] + sig * self.noise_S[:, np.newaxis])
        ).sum(1)
        ...
        plt.subplot(221)
        plt.hist(
            self.N_spikes_C[idxes] / (T / self.f),
            np.linspace(0, 5, 101),
            facecolor="g",
            alpha=0.5,
        )
        ...
        plt.subplot(222)
        plt.scatter(
            self.data["SNR"][idxes], self.N_spikes_S[idxes] / (T / self.f), c="g"
        )
        plt.scatter(
            self.data["SNR"][~idxes], self.N_spikes_S[~idxes] / (T / self.f), c="r"
        )
        # plt.ylim([0,10])

        plt.subplot(223)
        plt.plot(
            self.data["r_values"][idxes],
            self.N_spikes_S[idxes] / (T / self.f),
            "g.",
            markersize=1,
        )
        plt.plot(
    ...
    def plot_thesis(self, n, sig=[0, 1, 2], SNR_thr=2, r_thr=0):
        ...
        for i in range(2):
            ...
            ax_C_zoom.plot(
                t_arr[zrange], self.data["C"][n[i], zrange], "k", linewidth=0.5
            )
            ...
            ax_S_zoom.fill_betweenx(
                y_arr, x1, x2, facecolor="r", alpha=1, edgecolor=None
            )
            ax_S_zoom.vlines(
                t_arr[zrange],
                np.zeros(zlen),
            ...
            for j, s in enumerate(np.flipud(sig)):
                ax_S_zoom.plot(
                    t_arr[zrange],
                    np.ones(zlen) * self.baseline_S[n[i]] + s * self.noise_S[n[i]],
                    color=np.array([1, 0.3 * (2 - j), 0.3 * (2 - j)]),
                    linestyle=ls_arr[j],
                    label="$\\theta_S = %d$" % s,
                    linewidth=0.8,
                )
        ...
        for j, s in enumerate(sig):
            self.N_spikes_S = np.floor(
                self.data["S"]
                / (self.baseline_S[:, np.newaxis] + s * self.noise_S[:, np.newaxis])
            ).sum(1)
        ...
        self.N_spikes_S = np.floor(
            ...
            / (
                ...
                + s_adapt[:, np.newaxis] * self.noise_S[:, np.newaxis]
            )
        ).sum(1)
