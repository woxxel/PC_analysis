...
from .neuron_matching.matching import build_remap_from_shift_and_flow, get_shift_and_flow
...
class test_silence_in_sessions:
    ...
    def __init__(self,mousePath,
        ...
        # paths = [self.match.data[s]['filePath'] for s in self.match.data if isinstance(s,int)]
        # paths = replace_relative_path(paths,self.match.paths['data'])
        # self.paths = {
            # 'sessions': paths
        # }
    ...
    def run_all(self,n_processes=4,plt_bool=False,complete_new=False):
    ...
    def obtain_footprints(self,s,max_diff=None,complete_new=False):
        ...
        dims = self.dims
        T = 8989
        self.dataIn['A'] = scipy.sparse.csc_matrix((np.prod(self.dims),self.cluster.data['nC']))
        self.dataIn['C'] = np.random.rand(self.cluster.data['nC'],T)

        if complete_new:
            self.idxes['undetected'] = np.ones(self.cluster.data['nC'],'bool')
            self.dataIn['b'] = np.random.rand(np.prod(dims),1)
            self.dataIn['f'] = np.random.rand(1,T)
        else:
            ## find silent neurons in session s
            self.idxes['undetected']= self.cluster.status['clusters'] & np.isnan(self.cluster.matching['IDs'][:,self.s])

            print(self.idxes['undetected'])
            print('undetected:', self.idxes['undetected'].sum())
            ...
            c_idx = np.where(np.isfinite(self.cluster.matching['IDs'][:,s]))[0]
            n_idx = self.cluster.matching['IDs'][c_idx,s].astype('int')
            ...
            # self.dataIn['A'][:,c_idx] =ld['A'][:,n_idx]
        ...
        s_ref = np.full((self.cluster.data['nC'],2),-1,'int')
        n_ref = np.full((self.cluster.data['nC'],2),-1,'int')
        ...
        for c in np.where(self.idxes['undetected'])[0]:
            ...
            if len(s_pre)>0:
                s_ref[c,0] = s_pre[-1] if (s-s_pre[-1]) <= max_diff else -1
                if s_ref[c,0]>=0:
                    n_ref[c,0] = self.cluster.matching['IDs'][c,int(s_ref[c,0])].astype(int)

            if len(s_post)>0:
                s_ref[c,1] = s_post[0] if (s_post[0] - s) <= max_diff else -1
                if s_ref[c,1]>=0:
                    n_ref[c,1] = self.cluster.matching['IDs'][c,int(s_ref[c,1])].astype(int)
        ...
        progress = tqdm(s_load.astype('int'),total=len(s_load),desc='Loading footprints for processing Session %d...'%(s+1))
        for s_ld in progress:
            ...
            self.dataIn['A'][:,np.where(s_ref==s_ld)[0]] += 1./abs(s_ld-s) * scipy.sparse.vstack([a/a.sum() for a in A_tmp.T]).T
    ...
    def prepare_CNMF(self,path_to_videos,n_processes=0):

        '''
            function to set up a CaImAn batch-processing instance
        '''

        ### find video-file first (provide path as input!)
        fname = ...
