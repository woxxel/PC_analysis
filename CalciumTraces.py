...
class CalciumTraces:
  ...
  def plot_thesis(self,n,sig=[2,3,4],SNR_thr=2,r_thr=0):
    ...
    col = np.array([1,0,0])
    for i in range(2):
      ax_C_zoom = plt.axes([0.075+(0.29*i),0.7,0.225,0.275])
      ax_C_zoom.plot(t_arr[zrange],self.data['C'][n[i],zrange],'k')
      ...
      ax_C.plot(t_arr,self.data['C'][n[i],:],'k')
      ax_C.set_yticks([])
      ...
      ax_S_zoom = plt.axes([0.075+(0.29*i),0.4,0.225,0.3])
      ...
      ax_S_zoom.vlines(t_arr[zrange],np.zeros(zlen),self.data['S'][n[i],zrange],colors='k')
      #ax_S_zoom.bar(t_arr,self.data['S'][n[i],:],width=1/self.f,facecolor='r')
      ax_S_zoom.plot(t_arr[zrange],np.ones(zlen)*self.baseline_S[n[i]],'r-')
      for j,s in enumerate(np.flipud(sig)):
        ax_S_zoom.plot(t_arr[zrange],np.ones(zlen)*self.baseline_S[n[i]]+s*self.noise_S[n[i]],color=np.array([1,0.3*(2-j),0.3*(2-j)]),linestyle='--',label='$\\theta_S = %d$'%s)
      ...
      if i==0:
        ...
        ax_S.vlines(t_arr,np.zeros(T),self.data['S'][n[i],:],colors='k')
        ax_S.set_yticks([])
        ax_S.set_xlim([0,self.T/self.f])
        ax_S.spines['top'].set_visible(False)
        ax_S.spines['right'].set_visible(False)
      else:
        ax_S_zoom.legend(loc='upper right',bbox_to_anchor=[0.9,0.9],fontsize=10)
    ...
    for j,s in enumerate(sig):
      ...
      ax_fr.hist(self.N_spikes_S[idxes]/(T/self.f),np.linspace(0,(0.5)**j,51),facecolor='g',alpha=0.5,label='SNR$\geq$%d'%SNR_thr)
      ax_fr.hist(self.N_spikes_S[~idxes]/(T/self.f),np.linspace(0,(0.5)**j,51),facecolor='r',alpha=0.5,label='SNR<%d'%SNR_thr)
      ...
      ax_fr.text(0.5**j*0.5,ymax*0.8,'$\\theta_S = %d$'%s,fontsize=10)
    ...
    ax_theory = plt.axes([0.7,0.75,0.25,0.225])
    x_arr = np.linspace(2,10,101)
    for j,t in enumerate([500,2000,10000]):
      ax_theory.plot(x_arr,1-np.exp(t*np.log(norm.cdf(x_arr))),color=np.array([0.3*j,0.3*j,1]),label='T=%d'%t)
    for j,s in enumerate(sig):
    ...
    ax_theory.set_ylabel('$p(\\theta_{ex} > x$)')
    ax_theory.spines['top'].set_visible(False)
    ax_theory.spines['right'].set_visible(False)
    
    ax_theory2 = plt.axes([0.7,0.4,0.25,0.225])
    T_arr = np.linspace(0,10000,1000)
    plt.plot(T_arr,norm.ppf((1-0.01)**(1/T_arr)),'b')
    plt.xlabel('$T$')
    plt.ylabel('$\\theta_S$')
